...
pub async fn page(State(state): State<AdminState>) -> Html<String> {
    ...
    let content = commands_page_html(&details);
...
pub async fn commands_fragment(State(state): State<AdminState>) -> Html<String> {
    let mut details = state.messaging.all_command_details();
    details.sort_by(|a, b| a.name.cmp(&b.name));
    Html(commands_inner_html(&details))
}

// ── Page HTML ─────────────────────────────────────────────────────

fn commands_page_html(details: &[MessageTypeDetail]) -> String {
    format!(
        r##"<div class="flex flex-col flex-1" id="page-commands">
  <div class="bg-k-surface border border-k-subtle rounded-lg overflow-hidden flex flex-col flex-1">
    ...
    <div class="flex-1 overflow-hidden" hx-get="/fragments/commands" hx-trigger="every 5s" hx-swap="innerHTML">
      {inner}
    </div>
  </div>
</div>"##,
        count = details.len(),
        inner = commands_inner_html(details),
    )
}

fn commands_inner_html(details: &[MessageTypeDetail]) -> String {
    if details.is_empty() {
        return r#"<div class="text-center text-k-muted py-8 text-xs">No command handlers registered</div>"#.to_string();
    }

    // Master list (left) + detail pane (right) for the first item.
  ...
  <div class="w-[280px] min-w-[280px] border-r border-k-subtle overflow-y-auto" id="cmd-master-list">
    {master_items}
  </div>
  <div class="flex-1 p-[18px] overflow-y-auto" id="cmd-detail-pane">
    {detail_pane}
  </div>
</div>
<script>
(function(){{
  ...
  const masterList = document.getElementById('cmd-master-list');
  const detailPane = document.getElementById('cmd-detail-pane');
  if (!masterList || !detailPane) return;
  masterList.addEventListener('click', function(e) {{
    const item = e.target.closest('[data-cmd]');
    if (!item) return;
    masterList.querySelectorAll('[data-cmd]').forEach(function(i) {{
      i.classList.remove('active');
      i.style.background = '';
      i.style.borderLeftColor = 'transparent';
      const n = i.querySelector('.mi-name');
      if (n) {{ n.classList.remove('text-k-gold'); n.classList.add('text-k-text'); }}
    }});
    item.classList.add('active');
    item.style.background = 'var(--c-gold-dim)';
    item.style.borderLeftColor = 'var(--c-gold)';
    const n = item.querySelector('.mi-name');
    if (n) {{ n.classList.add('text-k-gold'); n.classList.remove('text-k-text'); }}
    const name = item.getAttribute('data-cmd');
  ...
  function renderDetail(d) {{
    ...
    html += '<span class="inline-flex px-2 py-0.5 rounded-full text-[11px] font-mono bg-k-gold-d text-k-gold">' + d.handlers.length + ' handler' + (d.handlers.length !== 1 ? 's' : '') + '</span>';
    html += '</div>';
    ...
    if (d.handlers.length > 0) {{
      ...
      html += '<table><thead><tr><th>Component</th><th>Client ID</th><th>Load Factor</th><th class="text-right">Permits</th></tr></thead><tbody>';
      d.handlers.forEach(function(h) {{
        ...
        html += '<td class="font-mono text-xs">' + h.load_factor + '</td>';
...
fn master_list_html(details: &[MessageTypeDetail]) -> String {
    ...
    for (i, d) in details.iter().enumerate() {
        ...
        let active_cls = if is_active {
            r#" active" style="background:var(--c-gold-dim);border-left-color:var(--c-gold)"#
        } else {
        ...
        html.push_str(&format!(
            r#"<div data-cmd="{name}" class="flex items-center justify-between px-4 py-2.5 border-b border-k-subtle cursor-pointer border-l-2{active_cls}"><span class="mi-name font-mono text-xs {name_cls}">{name}</span><span class="font-mono text-[11px] text-k-muted">{count} handler{plural}</span>{dispatched}</div>"#,
...
fn detail_pane_html(d: &MessageTypeDetail) -> String {
  ...
  <span class="inline-flex px-2 py-0.5 rounded-full text-[11px] font-mono bg-k-gold-d text-k-gold">{count} handler{plural}</span>
</div>
...
fn handler_row_html(h: &HandlerDetail) -> String {
    format!(
        r#"<tr><td class="!text-k-text">{component}</td><td class="font-mono text-xs">{client}</td><td class="font-mono text-xs">{lf}</td><td class="font-mono text-xs text-right">{permits}</td></tr>"#,
...
/// Serializes details to a JSON array for the client-side master-detail JS.
